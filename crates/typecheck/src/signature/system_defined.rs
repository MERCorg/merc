...
///
/// The comparison worklist also gets [add_numeric_basic_sort_dependencies]
/// applied to it: the four numeric basic sorts' own Appendix-B equations use
/// `if`/`==` on each other purely as an implementation detail — `nat.mcrl2`'s
/// `pred` needs `if` on `Pos`, `pos64.mcrl2`'s successor needs `if`/`==` on
/// `Nat`, and so on — whether or not the user's specification ever mentions
/// the other sort itself. Left alone, such an instantiation would only be
/// generated by coincidence, when that other sort also happens to appear
/// (textually or through inference) in the user's own specification. This is
/// deliberately a fixed, hand-coded dependency rather than a scan of
/// `basics`' own content: a blanket scan would also pull in comparisons for a
/// basic sort no other sort's usage actually reaches (`Real`'s own `min`/`max`
/// use `if` on `Real` too, but nothing requires that if `Real` itself is
/// never used) — see `test_unused_basic_sort_has_no_comparison_equations`.
///
...
pub(crate) fn build_system_defined_specification(
    ...
    add_numeric_basic_sort_dependencies(&mut comparison_worklist, spec, encoding);
...
/// Extends `worklist` with the other basic sorts that a `Pos`/`Nat`/`Int`/
/// `Real` entry's own Appendix-B equations depend on internally, so a
/// comparison instantiation `basics` itself needs gets generated even when
/// nothing in the user's specification (textually or through inference)
/// otherwise reaches that sort — see [build_system_defined_specification]'s
/// doc comment.
///
/// The four numeric sorts nest as `{Pos, Nat} ⊆ Int ⊆ Real`: `Pos` and `Nat`
/// depend on each other directly (`nat.mcrl2`'s own `pred`/`sqrt` helpers use
/// `if` on `Pos`; `pos.mcrl2`/`pos64.mcrl2`'s own successor/predecessor
/// helpers use `if`/comparisons on `Nat`), `int.mcrl2` is defined in terms of
/// `Pos`/`Nat`, and `real.mcrl2` in terms of `Int` (its own constructor takes
/// an `Int` argument). The dependency only ever points from a "larger" sort
/// down to a "smaller" one — using `Pos`/`Nat` alone never needs `Int`/`Real`
/// — so an unused `Real` still gets no comparison equations of its own; see
/// `test_unused_basic_sort_has_no_comparison_equations`. `Bool` needs nothing
/// extra: its own equations never do arithmetic. Under
/// [NumberEncoding::MachineWord], `Nat` additionally depends on `@word`
/// (`nat64.mcrl2`'s successor function is defined via `@word` digit
/// operations).
///
/// `spec`'s `sort_declarations` must already carry a resolved `SortId` for
/// `@word` (true from `build_system_defined_specification`/
/// `extend_system_with_inferred_sorts`'s own callers onward, once the
/// system-internal sorts are folded in): pushing an unresolved
/// [SortExpressionKind::Reference] for it instead would later hit
/// `sort_resolution.rs`'s "Names must have been resolved" panic, since
/// everything downstream of this point in the pipeline expects sorts to
/// already be `Simple`/`Resolved`.
fn add_numeric_basic_sort_dependencies(
    worklist: &mut Vec<SortExpression>,
    spec: &UntypedDataSpecification,
    encoding: NumberEncoding,
) {
    let has_sort = |worklist: &[SortExpression], sort: Sort| {
        worklist
            .iter()
            .any(|expr| matches!(&expr.node, SortExpressionKind::Simple(s) if *s == sort))
    };
    let has_word = |worklist: &[SortExpression]| {
        worklist
            .iter()
    ...
    // `Real` implies `Int`, which implies `{Pos, Nat}`; `Pos` and `Nat` imply
    // each other. Highest level present in `worklist` wins.
    let needs_core = [Sort::Pos, Sort::Nat, Sort::Int, Sort::Real]
        .into_iter()
        .any(|sort| has_sort(worklist, sort));
    let needs_int = [Sort::Int, Sort::Real].into_iter().any(|sort| has_sort(worklist, sort));

    if needs_core {
        push_sort(worklist, Sort::Pos);
        push_sort(worklist, Sort::Nat);
        // `Pos`/`Nat`'s own `<`/`<=` are themselves defined recursively via
        // `if` on `Bool` (`pos.mcrl2`: `@cDub(b,p) < @cDub(c,q) = if(c => b,
        // p < q, p <= q);`), so any numeric usage needs `Bool`'s comparison
        // equations too.
        push_sort(worklist, Sort::Bool);
    }
    if needs_int {
        push_sort(worklist, Sort::Int);
    }

    if encoding == NumberEncoding::MachineWord && needs_core && !has_word(worklist) {
        let word_id = spec
            .sort_declarations
            .iter()
        ...
        if let Some(word_id) = word_id {
            worklist.push(SortExpressionKind::Resolved("@word".to_string(), word_id).into());
        }
    }
}
...
/// [add_numeric_basic_sort_dependencies] included: a numeral literal's own
/// sort (`Pos`, almost always) is exactly the kind of inference-only sort
/// this function exists to catch, so this is the usual place the `Nat`/`@word`
/// dependency actually gets discovered from, not `build_system_defined_specification`'s
...
pub(crate) fn extend_system_with_inferred_sorts(
    ...
    add_numeric_basic_sort_dependencies(&mut comparison_covered, spec, encoding);
    expand_sorts(
        sources,
        comparison_covered,
        &mut comparison_seen,
...
/// a *trusted* declaration (Appendix B's own) may use one.
